...
def add_doodle_hat(image_path, output_path, pipeline,object_name, num_objects):
    # 读取图片
    ...
    if num_objects == 1:
        ...
        original_prompt = (
            f"Add {object_name} on the subject in the image. "
            f"These objects ({object_name}) MUST be drawn as a doodle, using only simple, minimal lines for their basic contours. NO details, NO filling, NO shading, NO color. Only pure line outlines for the {object_name} themselves."
            f"CRITICAL REQUIREMENT: You MUST NOT add any lines, outlines, or style changes to the subject or the background. "
            f"ALL other parts of the image—including the subject and background—MUST remain 100% IDENTICAL to the original photorealistic image. "
            f"Strictly preserve the original photorealistic style, all details, textures, and colors. "
            f"Absolutely NO stylistic or content changes to the subject or background are permitted."
        )
...
def convert_doodle_to_realistic(doodle_image_path, output_path, pipeline,object_name,num_objects):
    """将doodle图片转换为真实的图片"""
    # 读取doodle图片
    ...
    if num_objects == 1:
        ...
        original_prompt = (
            f"Convert the doodle-style {object_name} on the subject into realistic, photorealistic objects ({object_name})."
            f"CRITICAL REQUIREMENT: The rest of the image—including the subject's appearance, its exact colors, textures, and the entire background—MUST remain 100% IDENTICAL to the original base image (before any doodle was added). Strictly preserve the original photorealistic style, brightness, contrast, and color vividness of EVERYTHING except the {object_name}. Absolutely NO changes to the subject or background's original pixel data, only render the {object_name} realistically."
        )
    print(f"📝 原始编辑指令: {original_prompt}")
...
def main():
    ...
    for i, original_image_path in enumerate(all_image_paths):
        ...
        num_to_add = random.choice([1, 2])
        if num_to_add == 2 and len(objects_to_add) < 2:
            print(f"⚠️ 警告: JSON中的物体数量 ({len(objects_to_add)}) 不足2个，本次将只添加1个。")
            num_to_add = 1
        
        # 根据 num_to_add 生成 object_name 字符串
        if num_to_add == 1:
            object_name = random.choice(objects_to_add)
        else:
            # 使用 random.sample 确保抽取的2个物体不重复
            obj1, obj2 = random.sample(objects_to_add, 2)
            object_name = f"{obj1} and {obj2}"
            
        print(f"🎲 为此图片随机选择: {num_to_add} 个物体 ({object_name})")
        ...
        try:
            ...
            print("-"*60)
            add_doodle_hat(original_image_path, doodle_image_path, pipeline, random_object_name, num_to_add)
            
            # 第二步：将doodle转换为真实图片
            print("\n" + "-"*60)
            print(f"步骤 2/2: 将doodle转换为真实 ({random_object_name})")
            print("-"*60)
            convert_doodle_to_realistic(doodle_image_path, realistic_image_path, pipeline, random_object_name, num_to_add)
